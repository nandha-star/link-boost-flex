-- First, let's create secure database functions for payment operations
...
DROP POLICY IF EXISTS "Edge functions can insert purchases" ON public.connection_purchases;
DROP POLICY IF EXISTS "Edge functions can update purchases" ON public.connection_purchases;

-- Create more restrictive policies that only allow database functions to operate
-- Note: Edge functions with service role key will still bypass these, but this adds defense in depth
CREATE POLICY "Secure payment creation" ON public.connection_purchases
  FOR INSERT 
  WITH CHECK (
    -- Only allow inserts with pending status and valid user_id
    status = 'pending' AND 
    user_id IS NOT NULL AND
    amount > 0 AND
    connections_purchased > 0 AND
    stripe_session_id IS NOT NULL
  );

CREATE POLICY "Secure payment updates" ON public.connection_purchases
  FOR UPDATE 
  USING (
    -- Only allow status updates from pending to final states
    status = 'pending'
  )
  WITH CHECK (
    -- Only allow specific status transitions
    status IN ('paid', 'failed', 'cancelled') AND
    -- Don't allow modification of core payment data
    user_id = (SELECT user_id FROM public.connection_purchases WHERE id = connection_purchases.id) AND
    amount = (SELECT amount FROM public.connection_purchases WHERE id = connection_purchases.id) AND
    connections_purchased = (SELECT connections_purchased FROM public.connection_purchases WHERE id = connection_purchases.id)
  );